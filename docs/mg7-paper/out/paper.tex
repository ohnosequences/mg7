...
\emph{AA} and \emph{EPT} which serves as a way of defining and composing
...
\emph{AA} and \emph{EPT} with the goal of being a Scala-embedded
...
\emph{EK} and \emph{EPT} designed for the execution of embarrassingly
...
query sequence; we will call this the set of best hits of a query. Note
that the criteria which determines this set of best hits is fully
configurable, adjusted to conform to its meaning in each particular
dataset: those hits which we should take into account as valid for
assignment. This is of course experiment-dependent, and we can account
for read quality or technology-specific error profiles through
configuring these criteria for the set of best hits.

\paragraph{LCA Assignment}\label{lca-assignment}

The LCA assignment simply computes the lowest common ancestor of the set
of best hits. Other metagenomics analysis approaches
...
any way, as is customarily done in other cases.
