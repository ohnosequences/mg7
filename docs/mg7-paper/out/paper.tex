...
\item
  are annotated as being of \texttt{rRNA} type\footnote{We are aware of
    ...
    RNACentra, that we are \textbf{not using} due to a significant
...
\(S_1\); sequences which as a result end up having no assignments are
removed.

Once we have non-redundant set of RNA sequences containing all those
corresponding to 16S, we can apply our procedure for checking internal
consistency of taxonomic assignments.

We run MG7 using this set of sequences as query, and with reference all
but the one we are trying to assign. Explain

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  which kind of misassignments can be detected
\item
  how
\item
  why it is OK for sequences assigned to strange taxa, with no close 16S
  seq (they will get no hits)
\item
  we analyze every assignment independently
\end{enumerate}
...
other subsets of RNACentral; we do this for a 18S reference database.
